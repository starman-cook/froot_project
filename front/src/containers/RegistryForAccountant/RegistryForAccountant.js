...
import { fetchCancelPaid, fetchPaid, fetchTodaysFiles, fetchTodaysPayments } from '../../store/actions/paymentAction';
...
const RegistryForAccountant = () => {
    ...
    return (
        <div className="RegistryForAccountant">
            <h2 className="RegistryForAccountant__title">Платежи на сегодня: {today} </h2>
            <div className="RegistryForAccountant__content">
                <div className="RegistryForAccountant__total">
                    <h3 className="RegistryForAccountant__total-title">Общая сумма платежей по компаниям:</h3>
                    <div className="flex-column">
                        <span className="RegistryForAccountant__total-company">"Froot_Middle_Asia": <strong>{totalAsia}</strong> KZT</span>
                        <span className="RegistryForAccountant__total-company">"Froot_Бизнес": <strong>{totalBusiness}</strong> KZT</span>
                    </div>
                </div>
                <div className="RegistryForAccountant__downloads flex-column">
                    <a className="RegistryForAccountant__downloads-btn" download href={'http://localhost:8000/files/'+today+'.xlsx'}>Формирование Excell</a>
                    <a className="RegistryForAccountant__downloads-btn" onClick={downloadFiles}>Скачать все файлы</a>
                </div>            
            </div>
